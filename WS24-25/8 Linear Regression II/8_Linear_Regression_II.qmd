...
-   We want a model that is as close to the real parameters as possible
    ...
    **R²** tells us how much of the total variance in the outcome (dependent variable) can be explained by the predictor(s) (independent variables) in a statistical model.
    ...
    -   Therefore, we want to build a model such that it minimizes $Var(\varepsilon_i)$.
...
-   **Cookie Quality**: Response variable, measured on a scale from 1 to 40.
...
cookie_quality <- 2 + (2 * experience) + (0.01 * oven_temperature) + 
...
$\hat{cookiequality} = \hat{\beta_0} + \hat{\beta_1} experience+ \hat{\beta_2} oventemperature+ \hat{\beta_3} ingredientquality+ \hat{\beta_4} decoratingskill+\hat{\beta_5} gender$

```{r}
attach(cookie_data)
model1 <- lm(cookie_quality ~ experience + oven_temperature + ingredient_quality+ decorating_skill + gender)
summary(model1)
```

Interpretation:

-   (Intercept) 1.716186\*\* –\> When all other values are zero (so 0 years of experience, 0 degrees Celsius baking temperature, lowest ingredient quality and lowest decorating skill and being female) the cookie quality is 1.72 points/40. This value is significant on the 95%-level.

-   experience 2.003886\*\*\* –\> keeping all other variables stable the cookie quality raises by 2 points with each additional year of baking experience. This value is highly significant on the 99%-level.

-   oven_temperature 0.0101557\*\*\* –\> keeping all other variables stable the cookie quality raises by 0.01 points with each additional degree Celsius in baking temperature. This value is highly significant on the 99%-level.

-   ingredient_quality 0.490217\*\*\* –\> keeping all other variables stable the cookie quality raises by 0.49 points with each additional level of ingredient quality. This value is highly significant on the 99%-level.

-   decorating_skill 0.304204 \*\*\* –\> keeping all other variables stable the cookie quality raises by 0.304 points with each additional level of decorating skill. This value is highly significant on the 99%-level.

-   genderMale -0.025599 –\> keeping all other variables stable the cookie quality lowers by 0.026 points if the baker is male and not female. This value is really small and not significant on any of the typical levels.

-   RMSE: 1.029 –\> can only be interpreted in comparison to model2

-   R²: When adjusting for the number of predictors the model is able to explain 93.95% of the variance in the cookie quality.

Model 2 - just one predictor

$\hat{cookiequality} = \hat{\beta_0} + \hat{\beta_1} experience$

```{r}
attach(cookie_data)
model2 <- lm(cookie_quality ~ experience)
summary(model2)
```

-   (Intercept) 6.74\*\* –\> When all other values are zero (so 0 years of experience) the cookie quality is 6.74 points/40 points. This value is significant on the 99%-level.

-   experience 2.003886\*\*\* –\> keeping all other variables stable the cookie quality raises by 2 points with each additional year of baking experience. This value is highly significant on the 99%-level.

<!-- -->

-   RMSE: 1.309 –\> can only be interpreted in comparison to model1

-   R²: 0.902 –\> model1 is able to explain 90.2 % of the variance in the data

***Model comparison:***

Model1 has the lower RMSE (1.029 \< 1.309) and the higher adjusted R² (0.9395 \> 0.902) Therefore model 1 is the preferred one when it comes to the better in-sample fit.

## Task 2

Set up a model to explain the decorating skill by the using other variables. What is the best model in terms of in-sample fit that you can set up?
...
```{r}
# full model using all variables as predictors to get an overview
model3 <- lm(decorating_skill ~ cookie_quality + experience + oven_temperature + ingredient_quality + gender)
summary(model3)
```

```{r}
#leaving out gender as it is not statistically significant
model4 <- lm(decorating_skill ~ cookie_quality + experience + oven_temperature + ingredient_quality )
summary(model4)
```

```{r}
#leaving out gender as it is not statistically significant as well as oven_temperature since it has the lowest explanatory power of all the significant predictors
model5 <- lm(decorating_skill ~ cookie_quality + experience + ingredient_quality )
summary(model5)
```

Out of the three models (model3, model4, model5) model4 is the best in terms of in-sample-fit. The RMSE is relatively similar for all models but the adjusted R² differs slightly and is highest for model4.

Still we can conclude that all of the models have low explanatory power for the decorating skill. Important influence factors might have been left out by accident.

By manually choosing and eliminating variables I can try to find a good model - but for a big dataset with a lot of variables I cannot do it by hand because:

-   that takes a lot of time

-   there is a high chance of me missing the perfect model

There are other ways like:

-   backward elimination ( start a full model and take out those with the highest p-values/lowest explanatory power) –\> can be automatized and also done manually (for simple models)

-   cross-valiadation (using a training sample to fit the model and a test sample to validate it) –\> we will look further into that in January