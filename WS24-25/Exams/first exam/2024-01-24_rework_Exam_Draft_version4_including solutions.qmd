...
Which of the following methods is typically **NOT** useful when it comes to reducing model complexity?
...
> **Answer:**
>
> c\) Increasing the number of features with techniques like feature engineering and feature expansion to capture more information about the data. -\> typically adds more complexity to the model
...
> **Answer:**
>
> b\. A model is fit on the training set, and the fitted model is used to predict the responses for the observations in the validation set.
...
d. To analyze the relationship between multiple dependent variables.
...
Which of the following is **not** considered good practices in data analysis and coding?
...
**2.1.4. After applying the code: Are any values left that could potentially lead to problems? If so, how could we fix that problem (answer verbally, no code needed)? (2p)**
...
#### 4.1 Regularization: Ridge and Lasso regression (6 points)
...
> **Answer:** In Lasso regression, when variables income_inequality and median_income are highly correlated, one of the coefficients (β1 or β3​) may be reduced to exactly zero, effectively excluding one of the correlated predictors from the model.
...
Given the following dataset for predicting happiness score (HappyScore) based on GDP and IncomeInequality (all variables are categorical variables), **draw the decision tree you would construct for predicting HappyScore based on GDP and Income Inequality.** (6p)
