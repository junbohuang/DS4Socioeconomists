...
# Create a plot
ggplot(exam_score, aes(x = MathScore, y = after_stat(count), color = Gender_Factor)) +
...
```{r}
# plotting:
ggplot(exam_score, aes(x=grade, fill=factor_weekly_study_hours)) +
  geom_bar(position = "fill") +
  scale_y_continuous(n.breaks = 10) +
  labs(title = "Grade distribution for weekly study hours",
       x="grade", y="percentage", fill="study hours") +
  scale_fill_brewer(palette = "Dark2") + 
  theme_minimal()
