...
# 3. Make predictions on the test set using the trained model
out_predictions <- predict(model, newdata = data.frame(x_test), type="response") # type=response to get predicted probability and not log-odds

# 4. Calculate the out-of-sample RMSE, i.e. the test set RMSE.
residuals <- (y_test-out_predictions) # calculate the error (predicted values-observed values in the test set)
sqrt(mean(residuals^2)) # prints 0.4933515
...
p_class <- factor(ifelse(out_predictions > 0.5, 1, 0)) # creates a factor being 1 when predicted probability >0.5, 0 when <0.5
...
y_test_f <- factor(y_test, levels = c(1, 0))
tab <- table(predict = p_class, actual = y_test_f) # create table with predicted and acutal classes
