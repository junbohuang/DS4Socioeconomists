...
Visual Representation: #TODO: missing images
...
Is a linear regression where the house price is explained by the size of the lot, the presence of a garage and the age of the house.

```{r}
linear_model <- lm(formula = house_price ~ lot_size + garage_presence + house_age, data = data_houses)
...
```         
(Intercept)      323998.418*** --> a house with 0m², no garage and an age of 0years would cost approx.323,998.418€. The value is highly significant.

lot_size           1299.470*** --> keeping all other values stable, an increase in the size of the lot by 1m² would increase the price by approx. 1,299.47€. The value is highly significant.

garage_presence1  12705.039*** --> keeping all other values stable the presence of a garage increases the price by approx 12,705.039€. Here we compare garage=yes (1) to garage=no (0), so the case that no garage is present. The value is highly significant.

house_age         -3008.625*** --> keeping all other values stable an additional year in the house age, meaning the house being one year older would decrease the price by 3,008.625€. The value is highly significant.
```

Predicting the price:

```{r}
coef_linear_model <- coef(linear_model)
intercept_linear_model <- coef_linear_model[1]
b1_hat_linear_model <- coef_linear_model[2]  # coefficient for lot size
b2_hat_linear_model <- coef_linear_model[3]  # coefficient for garage presence
b3_hat_linear_model <- coef_linear_model[4]  # coefficient for house age
...
a.  The lot is 250m² big, the house has a garage and is 21 years old
    ...
    has_garage <- 1
    age <- 21
    has_garage_text <- ifelse(has_garage == 1, "has a garage", "has no garage")

    predicted_price <- intercept_linear_model + b1_hat_linear_model * size + b2_hat_linear_model * has_garage + b3_hat_linear_model*age
    predicted_price <- round(predicted_price, digits = 2)

    print(paste("The house that is", age, "years old, that", renovation_needed, "and is", size, "m² big would cost approximatly", predicted_price, "€."))
    ```
...
```         
glm(formula = house_sold ~ lot_size + garage_presence + house_age,family = binomial, data = data_houses)
```

Is a logistic regression where the chances of the house being sold within the first 14 days after listing is explained by the size of the lot, the presence of a garage and the age of the house.

```{r}
logistic_model <- glm(formula = house_sold ~ lot_size + garage_presence + house_age,family = binomial, data = data_houses)

library(margins)
margins_logistic_model <- margins(logistic_model)
summary(margins_logistic_model)
```

Interpretation of average marginal effects:

```         
garage_presence1  0.0350 ** --> The presence of a garage has an average marginal effect of 0.0350. The effect is significant.
When a garage is present the chances of the house being sold (..) increase by 3.5%

house_age -0.0042 *** --> With each year that the house is older its chances of being sold (...) decrease by 0.42%. This effect is highly significant.

lot_size  0.0008 *** --> With each additional m²  of the lot the chances of the house being sold (...) increase by 0.08%. This effect is highly significant.
```

Making predictions:

```{r}
coef_logistic_model <- coef(logistic_model)
intercept_logistic_model <- coef_logistic_model[1]
b1_hat_logistic_model <- coef_logistic_model[2]  # coefficient for lot size
b2_hat_logistic_model <- coef_logistic_model[3]  # coefficient for garage presence
b3_hat_logistic_model <- coef_logistic_model[4]  # coefficient for house age
...
a.  The lot is 250m² big, the house has a garage and is 21 years old
    ...
    # calculating log-odds
    selling_chance <- intercept_logistic_model + b1_hat_logistic_model * size + b2_hat_logistic_model * has_garage + b3_hat_logistic_model*age
    ...
    print(paste("The house that is", age, "years old, that", renovation_needed, "and is", size, "m² big, has a chance to be sold within the first 14 days after listing of approximatly", selling_chance_percent, "%."))

    ```

<!-- -->

a.  The lot is 55m² big, the house has no garage and is 49 years old
...
1.  Which subset of independent variables should be included in the model and why?
    ...
    variables_to_include = {d_heating, d_indoorfireplace, usd_cleaning_fee}

    we want to exclude d_hottub because its effect is insignificant with p \> 0.05

2.  What is the most profound factor that influence the rating according to this model?

    ```{r}
    model <- glm(highly_rated ~ usd_cleaning_fee + d_indoorfireplace + d_heating, 
                 data = airbnbsmall, family = "binomial")
    ...
    **Answer**: from the table we can see that d_indoorfireplace has the most profound influence on the rating, according to our model.

3.  How much likely that an airbnb is highly rated if it has an indoor fireplace? what about heating?

    indoor fireplace: 12.3% more likely

    heating: 10.5% mode likely